...
namespace Skuld.DataStorages.Entity
{
	public class EFCoreSymbolPropertyStorageService : ISymbolPropertyStorageService
	{
		...
		async Task<Models.SymbolPropertyGroup> MergeGroup(IDataContext Context, string id, PropertyGroup group, Models.SymbolPropertyGroup existsGroup, DateTime Time)
		{
		...
		async Task MergeGroupHistory(IDataContext Context, string id, PropertyGroup group,Models.SymbolPropertyGroupHistory existsGroupHistory,DateTime Time)
		{
		...
		async Task UpdateGroup(IDataContext Context,Symbol symbol, PropertyGroup group, Dictionary<string, Models.SymbolPropertyGroup> curGroups)
		{
			...
			if (curGroup == null)
			{
				...
				curGroups[group.Name] = await MergeGroup(Context,id, group, null, time);
				await MergeGroupHistory(Context,id, group, null, time);
			}
			...
			else if (!group.Time.HasValue)
			{
				...
				if (ops.Length != vps.Length || ops.Zip(vps, (o, v) => o.row != v.row || o.prop != v.prop || o.value != v.value).Any(p => p))
				{
					...
					curGroups[group.Name] = await MergeGroup(Context, id, group, curGroup, time);
					await MergeGroupHistory(Context, id, group, null, time);
				}
			}
			//如果和当前组时间相同，需要合并
			...
			{
				curGroups[group.Name] = await MergeGroup(Context, id, group, curGroup, group.Time.Value);
				var existsGroupHistory = await Context.Set<Models.SymbolPropertyGroupHistory>().FindAsync(id, group.Name, group.Time.Value);
				await MergeGroupHistory(Context, id, group, existsGroupHistory, group.Time.Value);
			}
			//如果比当前组时间新，需要新建组
			else if (group.Time.Value > curGroup.Time)
			{
				curGroups[group.Name] = await MergeGroup(Context, id, group, curGroup, group.Time.Value);
				await MergeGroupHistory(Context, id, group, null, group.Time.Value);
			}
